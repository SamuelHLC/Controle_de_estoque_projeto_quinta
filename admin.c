 ...
 *
 * CORRECAO [5]: admin agora se comunica com o servidor via socket
 * em vez de acessar estoque.dat diretamente. Isso elimina o conflito
 * de dois processos gravando no mesmo arquivo simultaneamente.
 * O menu e comportamento visual sao identicos ao original.
 *
 * Opcodes usados:
 *   op=10  listar produtos
 *   op=11  cadastrar produto  (envia struct Produto completo)
 *   op=12  alterar nome       (req[1]=id, envia char[50])
 *   op=13  alterar preco      (req[1]=id, envia float)
 *   op=14  adicionar unidades (req[1]=id, req[2]=qtd)
 *   op=15  remover produto    (req[1]=id)
 */
...
static void buscar_lista() {
    ...
    if (recv(s, (char*)&total, sizeof(int), 0) <= 0) { total = 0; close(s); return; }

    if (total > 0) {
        int tam = sizeof(Produto) * total, lido = 0;
        char *ptr = (char*)lista;
        while (lido < tam) {
        ...
        switch (op) {
            ...
            case 1: {
                ...
                if (s >= 0) {
                    int req[3] = {11, 0, 0};
                    send(s, (char*)req, sizeof(req), 0);
                    send(s, (char*)&np, sizeof(Produto), 0);
                    memset(resp, 0, sizeof(resp));
                    int n = recv(s, resp, 29, 0);
                    if (n > 0) resp[n] = '\0';
                    close(s);
                    printf(" %s\n", resp);
            ...
            case 2:
                if (sel >= 0 && sel < total) {
                    ...
                    if (s2 >= 0) {
                        ...
                        send(s2, (char*)req, sizeof(req), 0);
                        send(s2, novo, sizeof(novo), 0);
                        memset(resp, 0, sizeof(resp));
                        int n = recv(s2, resp, 29, 0);
                        if (n > 0) resp[n] = '\0';
                        close(s2);
            ...
            case 3:
                if (sel >= 0 && sel < total) {
                    ...
                    if (s2 >= 0) {
                        ...
                        send(s2, (char*)req, sizeof(req), 0);
                        send(s2, (char*)&novo, sizeof(float), 0);
                        memset(resp, 0, sizeof(resp));
                        int n = recv(s2, resp, 29, 0);
                        if (n > 0) resp[n] = '\0';
                        close(s2);
